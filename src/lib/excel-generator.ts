...
// Пълна педагогическа справка — една таблица, автофилтър + замразен горен ред.
// (за админ/ЗДУД/директор). Датите са ISO текст (YYYY-MM-DD) → сортират се коректно.
export function generateStudentReportExcel(headers: string[], rows: (string | number)[][]) {
  const wb = XLSX.utils.book_new()
  ...
  XLSX.writeFile(wb, `справка_ученици_${d}.xlsx`)
}
