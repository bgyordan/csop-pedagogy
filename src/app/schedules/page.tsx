...
import { CalendarClock, BookOpen, GraduationCap, HeartPulse, Home, ArrowRight } from 'lucide-react'
...
export default async function SchedulesPage({
  ...
  // ── Паралелки (списък + класни + брой ученици) ──
  let classes: any[] = []
  if (tab === 'classes') {
              ...
              {classes.map((c, i) => (
                <tr key={c.id} className={`hover:bg-blue-50/40 transition-colors ${i % 2 === 1 ? 'bg-slate-50/40' : 'bg-white'}`}>
                  ...
                  <td className="px-4 py-2.5 text-right">
                    <Link href={`/classes/${c.id}/schedule`} className="inline-flex items-center gap-1 text-xs font-medium text-blue-600 hover:text-blue-800">
                      Отвори <ArrowRight size={13} />
                    </Link>
                  </td>
                </tr>
              ))}
              {classes.length === 0 && (
                <tr><td colSpan={4} className="px-4 py-8 text-center text-slate-400">Няма паралелки</td></tr>
              )}
            </tbody>
          </table>
        </div>
