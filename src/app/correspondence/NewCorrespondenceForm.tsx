...
// Умни кодове — автоматично попълване
...
export default function NewCorrespondenceForm({
  ...
  function changeDirection(d: 'incoming' | 'outgoing' | 'internal') {
    ...
    const defaultCode = QUICK_CODES[d][0]
    setFolderIndex(defaultCode)
  }
      ...
      <form onSubmit={handleSubmit} className="p-5 space-y-3">
        ...
        <div className="flex items-end gap-3">
          <div className="flex-1">
            ...
            <button type="button" onClick={() => setShowAllNom(!showAllNom)}
              ...
              <ChevronDown size={12} className={showAllNom ? 'rotate-180 transition-transform' : ''} />
              Всички...
            </button>
          </div>
                  ...
                  <div key={section}>
                    ...
                    {items.map(item => (
                      <button key={item.item_code} type="button" onClick={() => selectNomCode(item.item_code)}
                        ...
                        {item.retention_years && <span className="ml-1 opacity-40 text-[10px]">({item.retention_years}г.)</span>}
                      </button>
                    ))}
                  </div>
                ))}
        ...
        {smartCode?.type === 'staff' && (
          <div className="p-3 bg-purple-50 rounded-xl border border-purple-100 space-y-2">
            ...
            {subject && <div className="text-xs text-purple-700 font-semibold bg-white border border-purple-100 rounded-lg px-3 py-2">Относно: {subject}</div>}
          </div>
        )}
        ...
        {smartCode?.type === 'student' && (
          <div className="p-3 bg-blue-50 rounded-xl border border-blue-100 space-y-2">
            ...
            {subject && <div className="text-xs text-blue-700 font-semibold bg-white border border-blue-100 rounded-lg px-3 py-2">Относно: {subject}</div>}
          </div>
        )}
