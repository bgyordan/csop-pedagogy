...
export default function TemplatesClient({ templates: initial, canManage, staffId }: Props) {
  ...
  function DocRow({ t }: { t: Template }) {
    ...
    return (
      <div className="group flex items-center gap-3 px-4 py-2.5 rounded-xl border border-transparent bg-slate-50/60 transition-all duration-300 hover:bg-white hover:border-blue-200/70 hover:shadow-[0_2px_12px_rgba(15,34,64,0.05)] hover:translate-x-1">
      ...
      {activityItems.length > 0 && (
        <div>
          <div className="flex items-center gap-2 mb-3">
            ...
            <span className="text-[11px] text-slate-300">{activityItems.length}</span>
            <div className="flex-1 h-px bg-slate-100" />
          </div>
          <div className="flex flex-col gap-2">
            {activityItems.map(t => <DocRow key={t.id} t={t} />)}
          </div>
        </div>
      )}
      {/* ── Административни ── */}
      {adminItems.length > 0 && (
        <div>
          <div className="flex items-center gap-2 mb-3">
            ...
            <span className="text-[11px] text-slate-300">{adminItems.length}</span>
            <div className="flex-1 h-px bg-slate-100" />
          </div>
          <div className="flex flex-col gap-2">
            {adminItems.map(t => <DocRow key={t.id} t={t} />)}
          </div>
        </div>
      )}
      {/* ── Празно състояние ── */}
