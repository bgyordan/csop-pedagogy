...
import {
  ...
  Baby, Pencil, Check, UploadCloud, FileSpreadsheet, FileCheck,
} from 'lucide-react'
...
const CATEGORIES = ['Декларации', 'Протоколи', 'Административни', 'Заявления', 'Други']
...
function fileIcon(fileName: string) {
  ...
  if (ext === 'xls' || ext === 'xlsx') return <FileSpreadsheet size={18} className="text-emerald-600" />
  if (ext === 'pdf') return <FileCheck size={18} className="text-rose-600" />
...
export default function TemplatesClient({ templates: initial, canManage, staffId }: Props) {
  ...
  const countByCat = useMemo(() => {
    ...
    templates.forEach(t => { const c = t.category || 'Други'; m.set(c, (m.get(c) || 0) + 1) })
    return m
  }, [templates])
  ...
  }, [templates, search, activeCategory, pgOnly])

  function resetForm() {
    setTitle(''); setDescription(''); setPendingFile(null)
    setIsPg(false); setCategory('Декларации'); setShowUpload(false)
  }
  ...
  async function doUpload() {
    ...
    const isAdmin = category === 'Административни'
  ...
  const pill = (key: string, label: string, count: number | null) => (
    <button key={key} onClick={() => setActiveCategory(key)}
      className={`px-3 py-1.5 rounded-full text-sm whitespace-nowrap transition-colors ${
        activeCategory === key ? 'bg-[#0f2240] text-white' : 'bg-white text-slate-600 border border-slate-200 hover:bg-slate-50'
      }`}>
  ...
  return (
    <div className="space-y-4 max-w-4xl mx-auto">
      ...
      <div className="flex items-center gap-2">
        ...
        {canManage && !showUpload && (
          <button onClick={() => setShowUpload(true)}
            className="inline-flex items-center gap-1.5 px-4 py-2 rounded-full text-white text-sm font-medium hover:opacity-90 transition-all shrink-0"
      ...
      <div className="flex items-center gap-2 overflow-x-auto pb-1">
        {pill('all', 'Всички', templates.length)}
        {CATEGORIES.map(c => pill(c, c, countByCat.get(c) || 0))}
        ...
        <button onClick={() => setPgOnly(v => !v)}
          className={`inline-flex items-center gap-1.5 px-3 py-1.5 rounded-full text-sm whitespace-nowrap transition-colors ${
            pgOnly ? 'bg-violet-600 text-white' : 'bg-white text-slate-600 border border-slate-200 hover:bg-slate-50'
          }`}>
          <Baby size={14} /> ПГ
        </button>
      </div>
      ...
      {filtered.length === 0 ? (
        <div className="text-center py-14 px-4 bg-white rounded-2xl border border-dashed border-slate-300">
          ...
          {templates.length === 0 ? (
            <p className="text-sm text-slate-500">Още няма образци{canManage ? ' — качете първия с бутона „Качи"' : ''}.</p>
          ) : (
            <>
              <p className="text-sm text-slate-500">Няма образци по този филтър.</p>
