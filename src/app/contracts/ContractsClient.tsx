        ...
        <form onSubmit={handleSearch} className="flex items-center gap-2">
          <div className="relative">
            ...
            <input placeholder="Търсене по №, предмет, контрагент..." value={search}
              onChange={e => setSearch(e.target.value)} className="input pl-9 w-72 text-xs" />
          </div>
              ...
              {contracts.length === 0 ? (
                ...
                return (
                  <tr key={item.id}
                    ...
                    className={`cursor-pointer border-b border-slate-100 transition-colors ${idx % 2 === 0 ? 'bg-white hover:bg-slate-50' : 'bg-slate-50/40 hover:bg-slate-100/60'} ${isExpiring ? 'border-l-2 border-l-amber-400' : ''}`}>
                    ...
                    <td className="px-3 py-1.5">
                      <span className="text-xs text-slate-600 max-w-[160px] truncate block">{item.subject}</span>
                    </td>
                    <td className="px-3 py-1.5 text-xs text-slate-500 whitespace-nowrap">
                      ...
                      <div className="flex items-center gap-1.5">
                        {canEdit && (
                          <button type="button" onClick={() => setEditItem(item)}
                            className="p-1 rounded-lg text-slate-300 hover:text-[#0f2240] hover:bg-slate-100 transition-colors text-xs" title="Редакция">
                            ✏️
                          </button>
                        )}
                        {item.file_url ? (
                          <button type="button"
                            ...
                            className="inline-flex items-center gap-1 text-[10px] font-bold text-slate-400 hover:text-[#0f2240] transition-colors">
