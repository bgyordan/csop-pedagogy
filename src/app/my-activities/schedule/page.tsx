...
export default async function TherapistSchedulePage({
  ...
  const { data: enrollments } = studentIds.length > 0
    ? await supabase.from('student_enrollments')
        ...
        .in('student_id', studentIds).eq('academic_year_id', currentYear?.id)
    : { data: [] }
  const classByStudent: Record<string, { classId: string | null; className: string; form: string }> = {}
  ;(enrollments || []).forEach((e: any) => {
    classByStudent[e.student_id] = { classId: e.class_id, className: e.class?.name || '', form: e.education_form || 'daily' }
  })
  const classIds = [...new Set((enrollments || []).map((e: any) => e.class_id).filter(Boolean))]
  const { data: classSchedules } = classIds.length > 0
    ? await supabase.from('class_schedules').select('id, class_id')
  ...
  ;(classSlotsRaw || []).forEach((s: any) => {
    ...
    slotsBySchedule[s.schedule_id][`${s.day}-${s.period}`] = { name: s.subject?.name || '', allowsPullout: s.subject?.allows_pullout || false }
  })
  ...
  const { data: ifoSlotsRaw } = studentIds.length > 0
    ? await supabase.from('teacher_ifo_slots')
        ...
        .in('student_id', studentIds).eq('academic_year_id', currentYear?.id).eq('term', term)
    : { data: [] }
  ...
  ;(ifoSlotsRaw || []).forEach((s: any) => {
    ...
    ifoByStudent[s.student_id].push({ day: s.day, period: s.period, name: s.subject?.name || '', allowsPullout: s.subject?.allows_pullout || false })
  })
  ...
  ;(myStudents || []).forEach(s => {
    ...
    if (info?.form === 'ifo') {
      ...
      ifo.forEach(h => {
        ...
        Object.entries(THERAPIST_PERIOD_RANGE).forEach(([per, range]) => {
          if (overlaps(ifoRange, range)) grid[`${h.day}-${per}`] = { name: h.name, allowsPullout: h.allowsPullout }
        })
      })
    ...
    } else studentSchedule[s.id] = null
  })
  ...
  ;(otherSlots || []).forEach((s: any) => {
    ...
    if (sch.staff_id === target.id) return
    takenByOthers[`${s.student_id}-${s.day}-${s.period}`] = sch.staff ? `${sch.staff.first_name} ${sch.staff.last_name}` : 'друг терапевт'
  })

  const { data: mySchedule } = await supabase
    .from('therapist_schedules').select('id')
    .eq('staff_id', target.id).eq('academic_year_id', currentYear?.id).eq('term', term).maybeSingle()
  let mySlots: any[] = []
  if (mySchedule) {
    const { data } = await supabase.from('therapist_slots').select('day, period, student_id').eq('schedule_id', mySchedule.id)
    mySlots = data || []
  }
  ...
  return (
    <div className="p-4 md:p-8 max-w-5xl mx-auto">
      <Link href={viewingOther ? '/schedules?tab=therapists' : '/my-activities'} className="inline-flex items-center gap-1.5 text-sm text-slate-500 hover:text-slate-800 mb-6">
        <ArrowLeft size={15} /> {viewingOther ? 'Назад към Разписания' : 'Назад към моите дейности'}
      </Link>
      {viewingOther && (
        <div className="mb-4 flex items-center gap-2 px-4 py-2.5 rounded-xl bg-amber-50 border border-amber-200 text-amber-800 text-sm">
          <Eye size={15} /> Преглед и редакция на чужд график (като управляващ)
        </div>
      )}
      <div className="mb-6">
      ...
      <TherapistScheduleGrid
        ...
        targetStaffId={viewingOther ? target.id : undefined}
      />
    </div>
  )
}
